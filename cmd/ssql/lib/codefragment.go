...
// commentSafe makes a command line safe inside the header's block
// comment: a value containing "*/" would end the comment and the rest of
// the header would compile as code (found by the injection equivalence
// case, 2026-09-22: `update -set x "fine */"` broke the generated
// program). The sequence is split with a space; the header is
// documentation, not something that is re-parsed.
...
func AssembleCodeFragments(input io.Reader) (string, error) {
	...
	if len(commands) > 0 {
		...
		for _, cmd := range commands {
			code.WriteString(commentSafe(cmd) + " |\n")
		}
