...
- [x] **SSH window-size propagation** (shell/v0.2.2, ssh/v0.1.11, ssql v4.44.6, 2026-05-19). `autocli/ssh` parses pty-req + window-change SSH payloads and pushes (cols, rows) through `shell.Options.ResizeChan`; `shell.Serve` calls `Terminal.SetSize`. Wide rows render at full terminal width; resize mid-session works.
- [x] **Handler IO migration** (ssql commit 340e025, 2026-05-19). All ssql command handlers now use `ctx.Stdin()`/`ctx.Stdout()`/`ctx.Stderr()` instead of `os.Stdin`/`os.Stdout`/`os.Stderr` directly. Zero behaviour change for bash users (default Context routes to os.*); unblocks Position 2 pipes for `ssql serve`. From*-handlers, generate*-codegen, file-only sinks (chart/explore/animate/parquet/xlsx) and helpers that print to server-stderr are intentionally left as `os.*` — they don't appear in serve pipes.
- [x] **`os.Exit` audit in command handlers** (2026-05-20). All 49 occurrences inspected; all are inside code-emission templates that emit `os.Exit(1)` into generated standalone Go programs (correct — generated programs are one-shot). No handler-direct `os.Exit` calls exist, so Position 2 pipes are safe from stages terminating the process mid-flight.
