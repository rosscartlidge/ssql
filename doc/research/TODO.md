...
- [ ] **Pipeline-aware completion via `SSQLGO=schema`** (see [schema-aware-completion.md](schema-aware-completion.md)). Each command declares a `SchemaOp` rule (input fields + argv → output fields). Same rule serves two runtimes: bash invokes `SSQLGO=schema <upstream> | ssql generate schema` as a subprocess; autocli-shell calls the rule in-process for instant pipeline-aware completion (`from-loaded | rename name=person | group-by <TAB>` → offers `person`). 4 phases, ~4 days total. Tested via a fourth `schema` mode added to `cmd/ssql/corpus_test.go` that shadow-asserts against runtime `_schema` headers.
