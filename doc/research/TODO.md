...
- [ ] **Remote Go execution** — see `remote-go-execution-proposal.md` (rev 3: single-mode, ssql-required-on-remote). When the SSH target has ssql installed (already required for `--` pushdown), ship a small `.ssql` script (~500 B) and run `ssql generate go -script -run` on the remote. Remote does the typed-parallel CSV/Parquet parse + transform + aggregate; only the (small) result rows return over SSH. Unlocks the v4.40 typed-parallel speedups for `from ssh` / `from catalog` workflows. Builds on the codegen-wrapper proposal's `-script PATH` flag. Phase A (single host): ~1 day. Phase B (catalog extension, parallel per-shard): ~1 day. Total ~2 days. Hosts without ssql gracefully fall back to the existing SSH-streams-bytes path (no acceleration but pipeline still works).
