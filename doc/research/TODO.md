...
- [ ] **Remote Go execution Phase C — catalog extension** (v4.43, see `catalog-remote-go-proposal.md`). Per-shard ship-and-run via the codegen-symmetric path; embedded .ssql template substituted with each shard's path at runtime; parallel orchestration (configurable concurrency cap); shards return JSONL with `_schema` headers; local merges and emits a single combined stream. ~2 days. Optional Phase C-bis (~half day) extends parallelism to today's CLI-baseline `from catalog` (no codegen) too — symmetric across all three modes.
