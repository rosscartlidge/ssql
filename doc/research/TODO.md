...
- [ ] **Remote Go execution** — see `remote-go-execution-proposal.md` (rev 2: .ssql-script-as-unit). When the SSH target has ssql installed (already required for `--` pushdown), ship a small `.ssql` script (~500 B) and run `ssql generate go -script -run` on the remote. Remote does the typed-parallel CSV/Parquet parse + transform + aggregate; only the (small) result rows return over SSH. Unlocks the v4.40 typed-parallel speedups for `from ssh` / `from catalog` workflows. Builds on the codegen-wrapper proposal's `-script PATH` flag. Phase A (Mode A, ssql-on-remote, single host): ~1 day. Phase B (capability cache + Go-on-remote fallback for Go-only hosts): ~1 day. Phase C (catalog + cross-compile binary fallback for hosts with neither): ~1-2 days.
