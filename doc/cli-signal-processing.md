# Signal Processing with ssql

A guided tour of ssql's signal commands — `fft`, `ifft`, `convolve`,
`correlate`, `spectrogram` — on the CPU you already have: frequency
analysis, filtering, smoothing, pattern detection, and how frequencies
change over time. Everything here runs on a laptop in well under a
second; the optional GPU build, for signals of millions of samples, is
the last section.
...
time series. It assumes that codelab: `from … | … | to table`, `where`,
...
9. [Performance Tips and the GPU Build](#performance-tips-and-the-gpu-build)

---

## Setup

**What you need.** `ssql` on your PATH, installed as in the
[CLI Codelab's Setup](cli-codelab.md#1-setup) (any of its install routes
works — the signal commands are in every build). `python3`, because the
test signals below are generated by short Python scripts, so the tutorial
needs no data files: Ubuntu and Debian have it (`python3 --version`; if
not, `sudo apt-get install -y python3`), macOS needs `brew install
python`. A browser for the charts.

**Work in an empty directory.** Every block writes its files — signals,
scripts, charts — where you are, and later blocks read what earlier ones
wrote:

```bash
# codelab: skip — the runner provides its own throwaway directory
mkdir -p ~/signals && cd ~/signals
```

**Charts.** Blocks end in `to chart … -output NAME.html`; open the file
with `xdg-open NAME.html` (Linux), `open NAME.html` (macOS), or File →
Open. They are written here, beside your data, not in `/tmp` — a browser
installed as a snap cannot see `/tmp` files written by other programs.

Check the two tools answer:
...
echo "Open signal_time.html (xdg-open / open, see Setup) to see the signal"
```
...
```bash
# Magnitude (default) - linear scale (three rows of each, to see the scale)
ssql from chirp.csv | \
...
ssql reads and writes WAV files itself. For MP3 and other formats, use
`ffmpeg` to convert first — the `ffmpeg` blocks below are the only ones
in this tutorial that need something beyond ssql and python3, and the
runner skips them; everything else here runs on the `song.wav` written
earlier.
...
## Performance Tips and the GPU Build

Everything above ran on the CPU. The signal commands also have a CUDA
implementation, `ssql_gpu`, that wins once a signal is large: FFT/IFFT
from about 16K samples (28-54× faster), convolution and correlation from
a 16-point kernel or window (4-500×). Below those sizes `ssql_gpu` uses
the CPU path itself, because copying the data to the GPU costs more than
the work. If your signals are tens of thousands of samples or fewer,
you can stop reading here.

### 1. Building ssql_gpu

The GPU build compiles CUDA code, so — unlike everything else in this
tutorial — it needs the source: the prebuilt binaries and `go install`
are CPU-only. Requirements: an NVIDIA GPU with CUDA support, and either
Docker with nvidia-container-toolkit or a local CUDA toolkit.

**Method 1: Docker Build (Recommended - no local CUDA needed)**

```bash
# codelab: skip — GPU build needs Docker + nvidia-container-toolkit
# Clone the repository (the CUDA code is built from source)
...
### 4. Use -max-lag for Autocorrelation

When searching for periodicity, limit the search range:

```bash
# Every lag (the whole signal length): more work than you usually need
ssql from periodic.csv | ssql correlate -field value -auto | ssql count

# Only the lags you care about: O(n × maxLag), and the peaks you want are early
ssql from periodic.csv | ssql correlate -field value -auto -max-lag 100 | ssql count
```

### 5. Filter Before FFT When Possible
...
### 7. Compiling Generated Code with GPU Support
